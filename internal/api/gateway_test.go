...
func (ts *APITestSuite) TestGateway() {
	...
	ts.T().Run("Create", func(t *testing.T) {
		...
		t.Run("List", func(t *testing.T) {
			...
			assert.NoError(storage.CreateOrganization(ts.DB(), &org2))
			...
			assert.NoError(storage.CreateGateway(ts.DB(), &gw2))
			...
			t.Run("List as org user", func(t *testing.T) {
				...
				_, err := storage.CreateUser(ts.DB(), &user, "password123")
				assert.NoError(err)
				...
				assert.NoError(storage.CreateOrganizationUser(ts.DB(), org.ID, user.ID, false))
		...
		t.Run("GetLastPing", func(t *testing.T) {
			...
			gw, err := storage.GetGateway(ts.DB(), lorawan.EUI64{8, 7, 6, 5, 4, 3, 2, 1}, false)
			assert.NoError(err)
			...
			assert.NoError(storage.CreateGateway(ts.DB(), &gw3))
			...
			assert.NoError(storage.CreateGatewayPing(ts.DB(), &ping))
			ping.CreatedAt = ping.CreatedAt.Truncate(time.Millisecond)

			gw.LastPingID = &ping.ID
			assert.NoError(storage.UpdateGateway(ts.DB(), &gw))
			...
			for i := range pingRX {
				assert.NoError(storage.CreateGatewayPingRX(ts.DB(), &pingRX[i]))
			}
