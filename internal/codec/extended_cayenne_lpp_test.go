...
func TestExtendedCayenneLPPDecode(t *testing.T) {
	Convey("Given a set of tests", t, func() {
		tests := []struct {
			Name   string
			Bytes  []byte
			Struct ExtendedCayenneLPP
		}{
			{
			...
			{
				Name:  "2 counter input",
				Bytes: []byte{3, 4, 0, 100, 5, 4, 3, 232},
				Struct: ExtendedCayenneLPP{
			...
			{
				Name:  "2 battery voltage",
				Bytes: []byte{3, 255, 1, 32, 5, 255, 1, 104},
				Struct: ExtendedCayenneLPP{
